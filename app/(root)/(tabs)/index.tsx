         ...
         justify-between mt-5">
              <View className="flex flex-row items-center">
                ...
                <View className="flex flex-col items-start ml-2 justify-center">
                  ...
                  <Text className="text-base font-rubik text-black-300">{user?.name}</Text>
                </View>
              </View>
            ...
            <View className="my-5">
              ...
              {latestPropertiesLoading ? <ActivityIndicator size='large' className="text-primary-300" /> : !latestProperties || latestProperties.length === 0 ? <NoResults /> : (
                <FlatList
                  ...
                  renderItem={({ item }) => <FeaturedCard item={item} onPress={() => handleCardPress(item.$id)} />} keyExtractor={(_, index) => index.toString()} horizontal bounces={false} showsHorizontalScrollIndicator={false} contentContainerClassName="flex gap-5 mt-5" />
              )}
